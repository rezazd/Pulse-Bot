...
async def show_profile(message_or_call):
...
async def manage_service(callback: types.CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        stmt = select(Service).where(Service.id == svc_id)
    ...
    if mz_res["status"] != 200:
        await callback.answer("❌ خطا در ارتباط با سرور مرزبان.", show_alert=True)
        return
...
async def get_subscription(callback: types.CallbackQuery):
    ...
    mz_res = await marzban_api.get_user(svc.marzban_username)
    if mz_res["status"] != 200:
        await callback.answer("❌ خطا در دریافت اطلاعات از مرزبان.", show_alert=True)
        return
