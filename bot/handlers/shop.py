...
@router.message(F.text.contains("خرید"), StateFilter("*"))
async def show_shop(message: types.Message, state: FSMContext):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not plans:
            return await message.answer("🛒 در حال حاضر هیچ پلنی برای فروش وجود ندارد.")
...
@router.callback_query(F.data.startswith("buy_plan_"), StateFilter("*"))
...
async def confirm_buy(callback: types.CallbackQuery):
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            mz_res = await marzban_api.add_user(username=username, data_limit=data_limit_bytes, expire=expire_timestamp)
            if mz_res["status"] == 200:
                new_service = Service(
                    user_id=user.id, marzban_username=username, plan_name=plan.name,
                    data_limit=data_limit_bytes, expire_date=expire_timestamp
                )
                ...
                success_text = (
                    ...
                    f"🔗 <b>لینک اتصال:</b>\n<code>{sub_link}</code>\n\n💡 <i>این لینک در بخش (پروفایل > سرویس‌های من) نیز در دسترس است.</i>"
                )
